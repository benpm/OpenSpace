...
Use the `windows-min` preset in `CMakePresets.json` (no tests, no vcpkg, minimal modules):
```bash
cmake --preset windows-min        # configure (Visual Studio 18 2026, RelWithDebInfo)
cmake --build --preset windows-min  # builds the OpenSpace target
```
- Output binary: `bin/RelWithDebInfo/OpenSpace.exe` (Qt DLLs deployed next to it via windeployqt).
- Qt6 is required (Core/Widgets/Network) by the Launcher GUI and AssetBuilder. Installed at
  `C:/Qt/6.8.3/msvc2022_64`; the preset points `CMAKE_PREFIX_PATH` there. To reinstall:
  `python -m aqt install-qt windows desktop 6.8.3 win64_msvc2022_64 --outputdir C:/Qt`.
- The `LNK4098: LIBCMT conflicts` warning and `Could NOT find WrapVulkanHeaders` are benign.
